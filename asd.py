...
class R3DClassifier(nn.Module):
    ...
    #         x = module(x)
    #         if name in ["conv1", "conv2", "conv3", "conv4"]:
    #         # if name in ["conv5"]:
    #             outputs.append(x)
    #     return outputs

    def forward(self, x):  # 原始训练网络
        # outputs = self.res3d(x)
        x = self.res3d(x)
...
if __name__ == "__main__":
    ...
    inputs = torch.rand(1, 1, 45, 45, 6)

    # net = R3DClassifier(3, (2, 2, 2, 2), pretrained=True)

    outputs = net.forward(inputs)
    # print(outputs.size())

# flops, params = profile(net, (inputs,))
# print("%.2fG" % (flops / 1000 ** 3), "%.2fM" % (params / 1000 ** 2))
